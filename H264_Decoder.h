...
typedef void(*h264_decoder_callback)(AVFrame* frame, AVPacket* pkt, void* user, AVCodecContext* codec_context);         /* the decoder callback, which will be called when we have decoded a frame */
...
public:
  ...
  bool load(float fps = 0.0f);                                     /* load a video file which is encoded with x264 */
  bool readFrame(unsigned char* data, int &size, bool &first_frame);                                                                      /* read a frame if necessary */

private:
  bool update(bool& needsMoreBytes, bool &first_frame);                                                     /* internally used to update/parse the data we read from the buffer or file */
  int readBuffer();                                                                      /* read a bit more data from the buffer */
  void decodeFrame(uint8_t* data, int size, bool &first_frame);                          /* decode a frame we read from the buffer */
